...
interface SubmitData extends Omit<MetaFields, 'edit' | 'value'> {}
...
export default function NewTemplate() {
    ...
    const onSubmit = async () => {
        let submitData = { ...defaultFields, metadata: metaFields };

        // remove edit fields from array object
        const metaData = submitData.metadata.map(field => {
            const { edit, ...rest } = field;
            return rest as SubmitData;
        });

        await mutateAsync({
            name: defaultFields[0]?.value as string,
            type: defaultFields[1]?.value as string,
    ...
    return (
        <Layout
            ...
            callToAction={
                <button
                    onClick={onSubmit}
                    className="bg-purple-500 hover:bg-purple-700 text-white text-md px-5 py-2 rounded"
                >
                    {isLoading ? <Spinner color="gray" /> : 'Save'}
                </button>
            }
        >
